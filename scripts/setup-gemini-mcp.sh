...
# MCP: codex, jina, claude-mem, asana, notion, box
set -euo pipefail
...
# box (http / OAuth)
BOX_CONFIG=$(jq -nc --arg url "$BOX_URL" '{url: $url, type: "http"}')
upsert_mcp_server "box" "$BOX_CONFIG"

for server_name in codex jina claude-mem asana notion box; do

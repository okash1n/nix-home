...
### FR-003c MCP 管理導線（ok-mcp-toggle）

- `make mcp` は MCP 自動同期を行わず、`ok-mcp-toggle` の管理導線（一覧表示と利用ヒント）を提供する。
- MCP の有効化/無効化/状態確認/事前認証は `agent-skills/ok-mcp-toggle/scripts/mcp_toggle.sh` で実施する。

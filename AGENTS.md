...
- 新しい詳細仕様（specs/）を追加する場合は、その判断経緯をADR（adr/）にも記録する
- 不明点や矛盾があれば作業前に質問する
...
- URL の内容把握・要約依頼では、最初に `ok-jina` スキルの `scripts/jina_ops.py read-url`（複数URL時は `parallel-read-url`）で本文を取得する。
- `ok-jina` による取得が失敗した場合のみ、`web.open` など他手段へフォールバックする。
- フォールバック時は、失敗理由（例: タイムアウト、接続エラー、空レスポンス）を回答に明記する。
- 回答には「本文取得に使った手段」「フォールバック有無」「根拠 URL」を必ず含める。
- `ok-jina`（`jina_ops.py`）またはフォールバック手段を実行していない状態で「本文を読んだ」「本文を直接取得した」と表現しない。

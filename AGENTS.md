...
- URL の内容把握・要約依頼では、最初に `mcp__jina__read_url` で本文を取得する。
- `mcp__jina__read_url` が失敗した場合のみ、`web.open` など他手段へフォールバックする。
- フォールバック時は、失敗理由（例: タイムアウト、接続エラー、空レスポンス）を回答に明記する。
- 回答には「本文取得に使った手段」「フォールバック有無」「根拠 URL」を必ず含める。
- `mcp__jina__read_url` を実行していない状態で「本文を読んだ」「本文を直接取得した」と表現しない。
